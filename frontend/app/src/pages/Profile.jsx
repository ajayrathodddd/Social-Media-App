  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        const { data: postsData } = await axios.get(`/api/posts/user/${parsedUser._id}`, config);
      ...
      <Row>
        <Col md="5">
          <Card className="mt-4 p-3">
            ...
            {/* Profile Picture Section fixed with getImageUrl */}
            ...
            <Form onSubmit={uploadProfilePictureHandler} className="mt-3">
              <Form.Group>
                <Form.Control
                  type="file"
        ...
        <Col md="7">
          <Card className="mt-4 p-3">
            <h3 className="text-center bg-light text-dark mt-2">Search Users</h3>
            ...
            <ListGroup className="mt-3">
              {results.map((result) => (
                <ListGroup.Item key={result._id} className="d-flex align-items-center gap-2">
                  <Link to={`/profile/${result._id}`}>{result.username}</Link>
                  <Button variant="success" size="sm" onClick={() => followUser(result._id)}>
                    Follow
                  </Button>
                  <Button variant="light" size="sm" onClick={() => startChartHandler(result._id)}>
                    Chat
                  </Button>
                </ListGroup.Item>
              ))}
            ...
            <Row>
              ...
              <Col md={6}>
                ...
                {user.following?.map((following, index) => {
                  const followingId = following._id || following;
                  return (
                  <Row className="g-2 mb-2" key={followingId.toString() || index}>
                    <Col>
                      <Card className="h-100 text-center p-2">
                        <Card.Body className="d-flex align-items-center p-0">
                          ...
                          <Button
                            variant="danger"
                            className="ms-auto btn-sm"
                            onClick={() => unfollowUser(followingId)}
                          >
                            Unfollow
                          </Button>
                        </Card.Body>
